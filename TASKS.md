# personal-profile — список задач

## 1. Инициализация проекта

- [x] Создать Angular-приложение (standalone, без NgModule)
- [x] Настроить структуру папок: `core`, `shared`, `features`
- [x] Подключить линтер и форматирование (ESLint, Prettier)
- [x] Добавить базовые скрипты в `package.json` (`start`, `build`, `test`)

## 2. Базовая архитектура

- [ ] Описать домен приложения: персональный профиль пользователя, для подключенеия в других проектах, расщиряется за счет плагинов
- [ ] Настроить маршрутизацию (`app.routes.ts`)
- [ ] Добавить layout (шапка, подвал, навигация, контент)
- [ ] Вынести конфигурацию окружения (`environment.ts`)

## 3. Управление состоянием

- [ ] Определить глобальное состояние (сервисы + signals)
- [ ] Избегать лишнего RxJS — только там, где нужны потоки (HTTP, WebSocket)
- [ ] Типизировать модели данных (`type`, не `interface`)

## 4. Первая фича - безопасность

- [ ] Создать папку `features/security/` (компонент, сервис, типы)
- [ ] Подключить фичу в роутинг - в навигации

## 6. Локализация

- [ ] Создать инструмент для переключения языка пользователя на основе @angular/localize
- [ ] Настрока каждого языка в отдельном файле
- [ ] Добавить поддержку китайского языка

## 8. API и данные

- [ ] Настроить `HttpClient` и interceptors (auth, ошибки)
- [ ] Описать типы ответов API
- [ ] Обработать состояния загрузки и ошибок (signals)

## 9. Безопасность

- [ ] Не хранить секреты в репозитории (`.env` в `.gitignore`)
- [ ] Санитизация пользовательского ввода
- [ ] Проверить зависимости на уязвимости (`npm audit`)

## 10. Качество и релиз

- [ ] Написать smoke-тесты для критичных сценариев
- [ ] Проверить сборку production (`ng build`)
- [ ] Обновить `Readme.md` (запуск, стек, структура)
- [ ] Настроить CI (сборка + тесты на push)

---

## Бэклог (после MVP)

## Темы

- [ ] Каждый контур имеет свою тему: ПРОД в палитре серого, ПРЕПРОД в палитре темно синего, LT в палитре желтого, ТЕСТ в палитре зеленого, ДЕВ - оранжевый. Все темы располагаются в отдельных файлах
- [ ] Добавь в профиль пользователя возможность переключать тему, тема должна меняться динамически.

## Бэклог (после MVP)

- [ ] Тёмная тема
- [ ] i18n (ru / en)
- [ ] PWA / офлайн
- [ ] E2E-тесты (Playwright / Cypress)
